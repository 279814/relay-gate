...
// TestListSamples_PageLimitDefaultAndCap pins the shared admin-list page
...
func TestListSamples_PageLimitDefaultAndCap(t *testing.T) {
	st := testStore(t)
	base := time.Now().UnixMilli()
	const n = 60
	for i := 0; i < n; i++ {
	...
	if len(def) != defaultPageLimit {
		t.Fatalf("omitted/zero limit: got %d samples, want default %d", len(def), defaultPageLimit)
	}

	_, err = st.ListSamples(SampleFilter{Limit: 100_000_000})
	if !errors.Is(err, ErrInvalidCursor) {
		t.Fatalf("huge limit error = %v, want ErrInvalidCursor (cap %d)", err, MaximumPageLimit)
	}

	atMax, err := st.ListSamples(SampleFilter{Limit: MaximumPageLimit})
	if err != nil {
		t.Fatal(err)
	}
	if len(atMax) != n {
