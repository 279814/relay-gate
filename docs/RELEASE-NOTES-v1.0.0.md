...
| P2 | docs/06 | Keyring、凭据 UI/轮换、最近错误、本地一行部署、公网 IP HTTPS **状态机 + 本地 fake Certbot**（PR #59 / #65）；Runtime Controller **maintenance 叠加态 + 暖机进度**；首次凭据 **bootstrap journal**；旧环境变量 **migration journal**、**reset-admin**、登录/Bearer **Argon2id**（仍兼容 env 明文）；**未**签发真实公网证书 |
...
- 「无环境变量时 ENCRYPTION_KEY / RELAY_KEYS 仅从 bootstrap/migrate 产物启动」（ADMIN_PASSWORD 已可仅靠哈希）
